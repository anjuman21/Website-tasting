# ShonenHub — Anime Fan Hub Hero Section

A fullscreen, cinematic hero section for a Naruto / Bleach / Attack on
Titan / One Piece fan hub. Built with React + TypeScript, Tailwind CSS,
and Lucide React icons.

⚠️ **No official studio footage, artwork, or logos are bundled.** The
video files referenced in `src/App.tsx` (`/public/videos/*.mp4`) are
placeholders — add your own recordings, fan-made edits, or properly
licensed clips before deploying.

## Features

- 4 crossfading looping background videos, one per series, each with
  its own accent color and copy
- Animated transparent PNG overlay ("train-bob" motion)
- Custom `.liquid-glass` frosted-glass UI treatment
- Per-series color theme swap (Naruto orange, Bleach cyan, AoT red,
  One Piece teal)
